...
The borders of the buttons are solid, thick and rounded. Now, when the [hover circle](#hover-circle) enters the button, it seems the border acts as a mask over it (— the hover circle).
...
One interesting feature is that, the hover circle expands almost fully when the mouse enters a button. Why do I say cheat? At first, I thought the circle should fill the button gradually as the mouse reaches the center. Hover your mouse on the button below from the edges to see what I mean.

- [ ] Add applet to demonstrate circle fill

You'll notice that, if the circle went gradually, it'll cause contrasting issues. For accessibility reasons, it makes sense the circle behaves as done on the GSAP website.
